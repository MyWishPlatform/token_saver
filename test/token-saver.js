...
contract("TokenSaver", async accounts => {
  ...
  for (let i = 0; i < totalERC20Contracts; i++) {
    ...
    it(
      "ERC20 №" + i + " should have 10000 tokens on main balance",
      async () => {
        const balance = await instanceERC20[i].balanceOf(accounts[0], {
          from: accounts[0]
        });
  ...
  it(
    "Correct number of ERC20 instances (should be " + totalERC20Contracts + ")",
    async () => {
      assert.equal(
        instanceERC20.length,
        totalERC20Contracts,
        "Total number of instances is incorrect"
      );
    }
  );
  ...
  it("Valid timestamp", async () => {
    ...
    //   "Time should be greater than now"
    // );
    assert(
  ...
  for (let i = 0; i < totalERC20Contracts; i++) {
    it(
      "Grant allowance to TokenSaver №" + i + " (should be 5000)",
      async () => {
        await instanceERC20[i].approve(tokenSaverAddress, 5000, {
          from: accounts[0]
        });
        await instanceERC20[i].allowance.call(
          accounts[0],
          tokenSaverAddress,
          {
            from: accounts[0]
  ...
  it("Add token types to Saver from incorrect address (should revert)", async () => {
    await assertRevert(
      instance.addTokenType(ERC20Array, { from: accounts[3] }),
      "Execution succeeded"
    );
  });
  ...
  it("Add token types to Saver (from backend)", async () => {
    await instance
      .addTokenType(ERC20Array, { from: accounts[0] })
      .then(function(result) {
        assert.equal(
          result.receipt.status,
          true,
          "Token type has been rejected (30 max)"
        );
      });
  });
  ...
  it("Try to add token that has been added before (should revert)", async () => {
    await assertRevert(
      instance.addTokenType(ERC20Array, { from: accounts[0] }),
  ...
  it("Try to add broken token address", async () => {
    await instance
      .addTokenType([accounts[5]], { from: accounts[0] })
      .then(function(result) {
        assert.equal(
          result.receipt.status,
          true,
  ...
  it("Transfer 500 tokens to Saver", async () => {
    for (let i = 0; i < eventList.length; i++) {
      await instanceERC20[i]
        .transfer(tokenSaverAddress, 500, { from: accounts[0] })
        .then(function(error, result) {});
      await instanceERC20[i].balanceOf(
        tokenSaverAddress,
        { from: accounts[0] },
        function(error, result) {
          if (!error) {
  ...
  for (let i = 0; i < totalERC20Contracts; i++) {
    it(
      "Check balance of reserve address №" + i + " (should have 5500)",
      async () => {
        await instanceERC20[i].balanceOf(
          accounts[1],
          { from: accounts[0] },
  ...
  it("Try to destroy contract from incorrect address (should revert)", async () => {
    await assertRevert(
      instance.selfdestruction({ from: accounts[4] }),
      "Execution succeeded"
    );
  });
