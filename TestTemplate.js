...
contract('TokenSaver', async (accounts) => {
    ...
    const ownerAddress = "D_OWNER_ADDRESS";
    const reserveAddress = "D_RESERVE_ADDRESS";
    const backendAddress = "D_BACKEND_ADDRESS";
    ...
    it('Execute token transfer on correct time', async () => {
        let now = new Date();
        advancement = timeStamp - Math.floor(Date.now() / 1000)+1000;
